...
let private fetchArticles session articlesView page =
    ...
    | Feed, Some s -> Cmd.OfAsync.perform Articles.fetchFeed {| Session = s; Offset = offset |} ArticlesFetched

    | TagFeed tag, _ ->
        Cmd.OfAsync.perform Articles.fetchArticlesWithTag {| Tag = tag; Offset = offset |} ArticlesFetched

    | _ -> Cmd.OfAsync.perform Articles.fetchArticles offset ArticlesFetched


let private fetchTags =
    Cmd.OfAsync.perform Tags.fetchTags () TagsFetched
...
let init session =
    ...
    { Articles = Loading
      ...
      ArticlesView = articlesView
      Session = session },
    Cmd.batch [ fetchArticles session articlesView 1
                fetchTags ]
...
let private banner =
    div [ ClassName "banner" ] [
        div [ ClassName "container" ] [
            ...
            p [] [
                str "A place to share your knowledge."
            ]
        ]
    ]
                ...
                div [ ClassName "col-md-9" ] [
                    ...
                    (match model.Articles with
                     ...
                     | Failure err ->
                         ...
                         str
                         <| sprintf "There was an issue fetching the articles"
