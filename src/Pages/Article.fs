...
let private fetchArticle session slug =
    match session with
    | Some s -> Cmd.OfAsync.perform Articles.fetchArticleWithSession {| Session = s; Slug = slug |} ArticleFetched
...
let private followAuthor session author =
    Cmd.OfAsync.perform Profiles.createFollower {| Session = session; Author = author |} FollowAuthorToggled

let private unfollowAuthor session author =
    Cmd.OfAsync.perform Profiles.deleteFollower {| Session = session; Author = author |} FollowAuthorToggled

let private favArticle authentication article =
    match authentication with
    | Authenticated ({ Session = s }) ->
        Cmd.OfAsync.perform Articles.favoriteArticle {| Session = s; Article = article |} FavoriteArticleToggled
    | _ -> Cmd.none
...
let private unfavArticle authentication article =
    match authentication with
    | Authenticated ({ Session = s }) ->
        Cmd.OfAsync.perform Articles.unfavoriteArticle {| Session = s; Article = article |} FavoriteArticleToggled
    | _ -> Cmd.none
...
let init session slug =
    ...
    { Article = Loading
      ...
      Errors = []
      Authentication = authentication },
    Cmd.batch [ fetchArticle session slug
                fetchComments slug
                cmdUser ]
...
let update msg model =
    ...
    | FollowAuthorToggled (Success author) ->
        let article =
            map (fun (article: FullArticle) -> { article with Author = author }) model.Article
...
let private commentForm dispatch (user: User) errors newComment =
    form [ ClassName "card comment-form"
        ...
        div [ ClassName "card-footer" ] [
            ...
            button [ ClassName "btn btn-sn btn-primary" ] [
                str "Post Comment"
            ]
        ]
    ]
...
let private articleOwnerButtons dispatch article =
    fragment [] [
        a [ href <| SessionRoute(EditArticle article.Slug)
