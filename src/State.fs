...
let private saveSession (session: Session) =
    Cmd.OfFunc.perform
        (fun s ->
            let sessionStr = Encode.Auto.toString (s, caseStrategy = CaseStrategy.CamelCase)
        ...
        | Login loginModel ->
            ...
            let (model, cmd) =
                ...
                | Pages.Login.ExternalMsg.SignedIn session ->
                    { model with Session = Some session }, Cmd.batch [ Route.Articles |> newUrl; saveSession session ]
