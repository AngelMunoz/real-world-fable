...
module Articles =
  ...
  let createComment
    ...
    safePost url (Decode.field "comment" Comment.Decoder) payload.Session {|
      comment = comment
    |}
  ...
  let fetchFavoriteArticles(author: Author) =
    let url =
      sprintf
        "%s?favorited=%s&limit=10&offset=%i"
        articlesBaseUrl
        author.Username
        0
...
module Users =
  ...
  let createUser
    (createUser:
      ...
      |})
    =
    post usersBaseUrl (Decode.field "user" Session.Decoder) {|
      user = createUser
    |}
  ...
  let fetchUser session =
    fetchUserWithDecoder User.Decoder session
